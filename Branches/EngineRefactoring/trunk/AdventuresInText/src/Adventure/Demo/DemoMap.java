...
import Adventure.Core.Condition.ComponentHasStatus;
import Adventure.Core.Condition.ComponentStatusInRange;
import Adventure.Core.Condition.ContainerHasItems;
import Adventure.Core.Condition.DialogHasRun;
import Adventure.Core.Condition.GameConditionComparisonType;
import Adventure.Core.Condition.HideableIsHidden;
import Adventure.Core.Condition.LocationHasExitDirection;
import Adventure.Core.Condition.PawnInLocation;
import Adventure.Core.Condition.PlayerMoveComparison;

import Adventure.Core.Objects.Actor;
import Adventure.Core.Objects.Dialog;
import Adventure.Core.Objects.Exit;
import Adventure.Core.Objects.Item;
import Adventure.Core.Objects.Location;
import Adventure.Core.Objects.Player;
...
public class DemoMap
    extends BaseMap
{
    @SuppressWarnings( "compatibility:4424693864107005201" )
    private static final long serialVersionUID = 1L;

    GameEngineComponentUtility componentUtility = Engine.getComponentUtility();
    /**
    ...
    public void buildMapLocations()
    {
        ...
        entryHall.addExit( componentUtility.getDirection( "south" ), mainRoom );

        mainRoom.addExit( componentUtility.getDirection( "north" ), entryHall );
        mainRoom.addExit( componentUtility.getDirection( "west" ), westWing );
        mainRoom.addExit( componentUtility.getDirection( "east" ), eastWing );

        westWing.addExit( componentUtility.getDirection( "east" ), mainRoom );
        // This exit is hidden
        Exit exit = new Exit( componentUtility.getDirection( "north" ), hiddenRoom );
        exit.hide();
        westWing.addExit( exit );

        hiddenRoom.addExit( componentUtility.getDirection( "south" ), westWing );

        eastWing.addExit( componentUtility.getDirection( "north" ), caveMouth );
        eastWing.addExit( componentUtility.getDirection( "south" ), southHall );
        eastWing.addExit( componentUtility.getDirection( "west" ), mainRoom );

        caveMouth.addExit( componentUtility.getDirection( "south" ), eastWing );

        southHall.addExit( componentUtility.getDirection( "west" ), exitCorridor );
        southHall.addExit( componentUtility.getDirection( "north" ), eastWing );

        exitCorridor.addExit( componentUtility.getDirection( "east" ), southHall );

        exitHall.addExit( componentUtility.getDirection( "north" ), exitCorridor );
    ...
    public void buildMapMechanics()
    {
        ...
        SpeakerInitiatesDialog narratorIntroducesSelf =
            new SpeakerInitiatesDialog( "OpeningDialog", componentUtility.getSpeaker( "Narrator" ) );

        // We need to add a check to make sure it is only run once
        narratorIntroducesSelf.addEventCondition( new DialogHasRun( "introComplete", componentUtility.getDialog( "N-Intro" ),
                                                                    true ) );

        // We also need to add a condition to make sure it is the first turn
        narratorIntroducesSelf.addEventCondition( new PlayerMoveComparison( "firstTurn", 0, GameConditionComparisonType.EQUAL ) );
        ...
        MovePawnToLocation narrator2exit =
            new MovePawnToLocation( "Narrator2Exit", componentUtility.getPawn( "Narrator" ), componentUtility.getLocation( "Exit Corridor" ) );
        ...
        DialogHasRun narratorSpeachDone =
            new DialogHasRun( "narratorSpeachDone", componentUtility.getDialog( "N-Intro-Ending" ) );
        narrator2exit.addEventCondition( narratorSpeachDone );

        // This condition confirms that the Narrator is still in the Entry Hall.
        narrator2exit.addEventCondition( new PawnInLocation( "narratorInEntry", componentUtility.getPawn( "Narrator" ),
                                                             componentUtility.getLocation( "Entry Hall" ) ) );
        ...
        SetSpeakerInitialDialog findKeysDialog =
            new SetSpeakerInitialDialog( "findKeysDialog", componentUtility.getSpeaker( "Narrator" ),
                                         componentUtility.getDialog( "N Exit Not Open" ) );
        ...
        findKeysDialog.addEventCondition( new ComponentHasStatus( "findKeysDialogActive",
                                                                  componentUtility.getComponent( "Narrator" ), "dialog changed",
                                                                  true ) );
        ...
        ChangeComponentDescription newWestWingDesc =
            new ChangeComponentDescription( "WWDescriptUpdate", componentUtility.getComponent( "West Wing" ),
                                            "This room appears to be a bedchamber of some kind, though it doesn't have any" +
                                            " furnishings. There is a slight echo as your footsteps fall on the stone floor." +
                                            " There is an exit on the north wall, and another to the west." );

        // We need a check to see if the hidden exit is found first.
        newWestWingDesc.addEventCondition( new HideableIsHidden( "hiddenRoomFound", componentUtility.getHideable( "n_30" ),
                                                                 true ) );

        // We also check that this event only happens 1 time
        newWestWingDesc.addEventCondition( new ComponentHasStatus( "WWDescUpdated", componentUtility.getComponent( "West Wing" ),
        ...
        InitiateDialog hiddenRoomDialog =
            new InitiateDialog( "hiddenRoomDialog", componentUtility.getDialog( "Hidden Room Found" ) );
        ...
        hiddenRoomDialog.addEventCondition( new ComponentHasStatus( "hasWWDescUpdated",
                                                                    componentUtility.getComponent( "West Wing" ),
                                                                    "description changed" ) );

        // We also check that this event only happens 1 time
        ComponentHasStatus wwDialogPlayed =
            new ComponentHasStatus( "wwDialogPlayed", componentUtility.getComponent( "Hidden Room Found" ), "has run", true );
        hiddenRoomDialog.addEventCondition( wwDialogPlayed );

        // This condition confirms that the Player is still in the West Wing.
        hiddenRoomDialog.addEventCondition( new PawnInLocation( "playerInWestWing", Engine.getCurrentPlayer(),
                                                                componentUtility.getLocation( "West Wing" ) ) );

        // Inside the hidden room there you can play a game of Paper/Rock/Scissors
        AssignStatusToComponent beginPlayingGame =
            new AssignStatusToComponent( "playingGame", componentUtility.getComponent( "Player" ), "PaperRockScissors", 1 );

        // Depending on the conversation with the Gamer in the hidden room you might get a status assigned
        DialogHasRun gamerPlayDialog = new DialogHasRun( "gamerPlayDialog", componentUtility.getDialog( "Gamer Play" ) );
        beginPlayingGame.addEventCondition( gamerPlayDialog );

        // The game can only be won once, so we check here
        ComponentHasStatus wonGame =
            new ComponentHasStatus( "wonGame", Engine.getCurrentPlayer(), "WonPaperRockScissors", true );
        ...
        ComponentHasStatus isPlayingGame =
            new ComponentHasStatus( "isPlayingGame", Engine.getCurrentPlayer(), "PaperRockScissors" );
        ...
        ItemMovedToNewContainer sneakTheft =
            new ItemMovedToNewContainer( "sneakTheft", componentUtility.getContainer( "Sneak" ), componentUtility.getItem( "Key 2" ), 1,
                                         Engine.getCurrentPlayer() );

        // We need to make sure that the player is in the South Hall first
        PawnInLocation playerInSouthHall =
            new PawnInLocation( "playerInSouthHall", Engine.getCurrentPlayer(), componentUtility.getLocation( "South Hall" ) );
        sneakTheft.addEventCondition( playerInSouthHall );

        // We also need to make sure the sneak is still hidden
        sneakTheft.addEventCondition( new ComponentHasStatus( "sneakIsHidden", componentUtility.getComponent( "Sneak" ),
                                                              "hidden" ) );

        // When the sneak is found, a dialog is started with this event
        InitiateDialog sneakFoundDialog = new InitiateDialog( "sneakFoundDialog", componentUtility.getDialog( "Sneak Found" ) );

        // We need to make sure the sneak is not hidden is not hidden anymore
        ComponentHasStatus sneakNotHidden =
            new ComponentHasStatus( "sneakNotHidden", componentUtility.getComponent( "Sneak" ), "hidden", true );
        ...
        ContainerHasItems sneakHasKey =
            new ContainerHasItems( "sneakHasKey", componentUtility.getContainer( "Sneak" ), componentUtility.getItem( "Key 2" ), 1 );
        sneakFoundDialog.addEventCondition( sneakHasKey );

        // We also need to make sure this dialog only runs once.
        ComponentHasStatus shDialogPlayed =
            new ComponentHasStatus( "shDialogPlayed", componentUtility.getComponent( "Sneak Found" ), "has run", true );
        ...
        ItemMovedToNewContainer sneakReturnsKey =
            new ItemMovedToNewContainer( "sneakReturnsKey", Engine.getCurrentPlayer(), componentUtility.getItem( "Key 2" ), 1,
                                         componentUtility.getContainer( "Sneak" ) );
        ...
        ComponentHasStatus shDialogHasPlayed =
            new ComponentHasStatus( "shDialogHasPlayed", componentUtility.getComponent( "Sneak Found" ), "has run" );
        ...
        AddExitToLocation addLevelExit =
            new AddExitToLocation( "addLevelExit", componentUtility.getLocation( "Exit Corridor" ), componentUtility.getLocation( "Exit Hall" ),
                                   componentUtility.getDirection( "South" ) );

        // We must make sure that the narrator is at the exit first
        addLevelExit.addEventCondition( new PawnInLocation( "narratorAtExit", componentUtility.getPawn( "Narrator" ),
                                                            componentUtility.getLocation( "Exit Corridor" ) ) );

        // We must make sure that the player is at the exit also
        addLevelExit.addEventCondition( new PawnInLocation( "playerAtExit", Engine.getCurrentPlayer(),
                                                            componentUtility.getLocation( "Exit Corridor" ) ) );

        // We must also make sure that the player has found both keys first.
        addLevelExit.addEventCondition( new ContainerHasItems( "playerHasKey1", Engine.getCurrentPlayer(),
                                                               componentUtility.getItem( "Key 1" ), 1 ) );
        addLevelExit.addEventCondition( new ContainerHasItems( "playerHasKey2", Engine.getCurrentPlayer(),
                                                               componentUtility.getItem( "Key 2" ), 1 ) );

        // We need to make sure this event only happens one time also
        LocationHasExitDirection exitPathNotAdded =
            new LocationHasExitDirection( "exitPathNotAdded", componentUtility.getLocation( "Exit Corridor" ),
                                          componentUtility.getDirection( "South" ), true );
        ...
        SetSpeakerInitialDialog keysFoundDialog =
            new SetSpeakerInitialDialog( "keysFoundDialog", componentUtility.getSpeaker( "Narrator" ),
                                         componentUtility.getDialog( "Narrator Exit Open" ) );
        ...
        keysFoundDialog.addEventCondition( new ComponentStatusInRange( "keysFoundActive",
                                                                       componentUtility.getComponent( "Narrator" ),
                                                                       "dialog changed", 1, 1 ) );

        // This event will trigger the Narrator's final dialog.
        InitiateDialog exitFoundDialog = new InitiateDialog( "exitFoundDialog", componentUtility.getDialog( "Narrator Exit" ) );

        // We need to make sure this dialog only runs once.
        ComponentHasStatus exitDialogPlayed =
            new ComponentHasStatus( "exitDialogPlayed", componentUtility.getComponent( "Narrator Exit" ), "has run", true );
        exitFoundDialog.addEventCondition( exitDialogPlayed );

        // We only want to initiate this dialog once the exit is added
        LocationHasExitDirection exitPathAdded =
            new LocationHasExitDirection( "exitPathAdded", componentUtility.getLocation( "Exit Corridor" ),
                                          componentUtility.getDirection( "South" ) );
        ...
        ChangeComponentDescription newExitCorridorDesc =
            new ChangeComponentDescription( "newExitCorridorDesc", componentUtility.getComponent( "Exit Corridor" ),
        ...
        // We also check that this event only happens 1 time
        newExitCorridorDesc.addEventCondition( new ComponentHasStatus( "exitDescUpdated",
                                                                       componentUtility.getComponent( "Exit Corridor" ),
                                                                       "description changed", true ) );
        ...
        ending.addEventCondition( new PawnInLocation( "playerAtEnd", Engine.getCurrentPlayer(),
                                                      componentUtility.getLocation( "Exit Hall" ) ) );
    }
}
