    ...
    extends BaseEvent
{
    ...
    public void processEvent( Operation operation )
    {
        ...
        GameActor gamer = ( GameActor ) Engine.getComponent( "Gamer" );
        
        ( Engine.getComponent( "Gamer Play" ) ).setStatus( "has run", 0 );
        ...
        if ( player.hasStatus( "PaperRockScissors" ) )
        {
            ...
            while ( winCount < 3 && loseCount < 3 )
            {
                ...
                try
                {
                    // we use a tyr block to watch for invalid input.
                    choice = IO.getChoice();
                }
                catch ( Exception e )
                {
                ...
                if ( winCount == 3 )
                {
                    ...
                    gamer.setInitialDialog( ( GameDialog ) Engine.getComponent( "Gamer Defeated" ) );
                    ( ( GameDialog ) Engine.getComponent( "Gamer Defeat" ) ).initiate();
                    return;
                }
                else if ( loseCount == 3 )
                {
