...
public class Loader
{
     ...
     * @param fileOrDirectory True if you also want to locate directores with the given name, false if you only want to search files.
     * @return The File object for the file with the given name if it is found, or null otherwise.
     */
    private static File getFileByName( String fileName, File workingDirectory, boolean fileOrDirectory )
    {
        ...
        if ( fileListing != null )
        {
            for ( File file : fileListing )
            {
                ...
                {
                    File temp = null;
                    if ( fileOrDirectory && file.getName().equalsIgnoreCase( fileName ) )
                    {
                        temp = file;
                    }
                    else
                    {
                        temp = getFileByName( fileName, file, fileOrDirectory );
                    }
