    ...
    extends BaseMap
{
    ...
    public void buildMapLocations()
    {
        ...
        entryHall.addExit( Engine.getDirection( "south" ), mainRoom );

        mainRoom.addExit( Engine.getDirection( "north" ), entryHall );
        mainRoom.addExit( Engine.getDirection( "west" ), westWing );
        mainRoom.addExit( Engine.getDirection( "east" ), eastWing );

        westWing.addExit( Engine.getDirection( "east" ), mainRoom );
        // This exit is hidden
        Exit exit = new Exit( Engine.getDirection( "north" ), hiddenRoom );
        exit.hide();
        westWing.addExit( exit );

        hiddenRoom.addExit( Engine.getDirection( "south" ), westWing );

        eastWing.addExit( Engine.getDirection( "north" ), caveMouth );
        eastWing.addExit( Engine.getDirection( "south" ), southHall );
        eastWing.addExit( Engine.getDirection( "west" ), mainRoom );

        caveMouth.addExit( Engine.getDirection( "south" ), eastWing );

        southHall.addExit( Engine.getDirection( "west" ), exitCorridor );
        southHall.addExit( Engine.getDirection( "north" ), eastWing );

        exitCorridor.addExit( Engine.getDirection( "east" ), southHall );

        exitHall.addExit( Engine.getDirection( "north" ), exitCorridor );
    ...
    public void buildMapMechanics()
    {
        ...
        SpeakerInitiatesDialog narratorIntroducesSelf =
            new SpeakerInitiatesDialog( "OpeningDialog", Engine.getSpeaker( "Narrator" ) );

        // We need to add a check to make sure it is only run once
        narratorIntroducesSelf.addEventCondition( new DialogHasRun( "introComplete", Engine.getDialog( "N-Intro" ),
                                                                    true ) );

        // We also need to add a condition to make sure it is the first turn
        narratorIntroducesSelf.addEventCondition( new PlayerMoveComparison( "firstTurn", 0,
                                                                            GameConditionComparisonType.EQUAL ) );
        ...
        MovePawnToLocation narrator2exit =
            new MovePawnToLocation( "Narrator2Exit", Engine.getPawn( "Narrator" ), Engine.getLocation( "Exit Corridor" ) );
        ...
        DialogHasRun narratorSpeachDone =
            new DialogHasRun( "narratorSpeachDone", Engine.getDialog( "N-Intro-Ending" ) );
        narrator2exit.addEventCondition( narratorSpeachDone );

        // This condition confirms that the Narrator is still in the Entry Hall.
        narrator2exit.addEventCondition( new PawnInLocation( "narratorInEntry", Engine.getPawn( "Narrator" ),
                                                             Engine.getLocation( "Entry Hall" ) ) );
        ...
        SetSpeakerInitialDialog findKeysDialog =
            new SetSpeakerInitialDialog( "findKeysDialog", Engine.getSpeaker( "Narrator" ),
                                         Engine.getDialog( "N Exit Not Open" ) );
        ...
        findKeysDialog.addEventCondition( new ComponentHasStatus( "findKeysDialogActive",
                                                                  Engine.getComponent( "Narrator" ), "dialog changed",
                                                                  true ) );
        ...
        newWestWingDesc.addEventCondition( new HideableIsHidden( "hiddenRoomFound", Engine.getHideable( "n_30" ),
                                                                 true ) );

        // We also check that this event only happens 1 time
        newWestWingDesc.addEventCondition( new ComponentHasStatus( "WWDescUpdated", Engine.getComponent( "West Wing" ),
        ...
        InitiateDialog hiddenRoomDialog =
            new InitiateDialog( "hiddenRoomDialog", Engine.getDialog( "Hidden Room Found" ) );
        ...
        ComponentHasStatus wwDialogPlayed =
            new ComponentHasStatus( "wwDialogPlayed", Engine.getComponent( "Hidden Room Found" ), "has run", true );
        ...
        DialogHasRun gamerPlayDialog = new DialogHasRun( "gamerPlayDialog", Engine.getDialog( "Gamer Play" ) );
        ...
        ComponentHasStatus wonGame =
            new ComponentHasStatus( "wonGame", Engine.currentPlayer(), "WonPaperRockScissors", true );
        ...
        ItemMovedToNewContainer sneakTheft =
            new ItemMovedToNewContainer( "sneakTheft", Engine.getContainer( "Sneak" ), Engine.getItem( "Key 2" ), 1,
                                         Engine.currentPlayer() );

        // We need to make sure that the player is in the South Hall first
        PawnInLocation playerInSouthHall =
            new PawnInLocation( "playerInSouthHall", Engine.currentPlayer(), Engine.getLocation( "South Hall" ) );
        ...
        InitiateDialog sneakFoundDialog = new InitiateDialog( "sneakFoundDialog", Engine.getDialog( "Sneak Found" ) );

        // We need to make sure the sneak is not hidden is not hidden anymore
        ComponentHasStatus sneakNotHidden =
            new ComponentHasStatus( "sneakNotHidden", Engine.getComponent( "Sneak" ), "hidden", true );
        ...
        ContainerHasItems sneakHasKey =
            new ContainerHasItems( "sneakHasKey", Engine.getContainer( "Sneak" ), Engine.getItem( "Key 2" ), 1 );
        ...
        ComponentHasStatus shDialogPlayed =
            new ComponentHasStatus( "shDialogPlayed", Engine.getComponent( "Sneak Found" ), "has run", true );
        ...
        ItemMovedToNewContainer sneakReturnsKey =
            new ItemMovedToNewContainer( "sneakReturnsKey", Engine.currentPlayer(), Engine.getItem( "Key 2" ), 1,
                                         Engine.getContainer( "Sneak" ) );
        ...
        AddExitToLocation addLevelExit =
            new AddExitToLocation( "addLevelExit", Engine.getLocation( "Exit Corridor" ), Engine.getLocation( "Exit Hall" ),
                                   Engine.getDirection( "South" ) );

        // We must make sure that the narrator is at the exit first
        addLevelExit.addEventCondition( new PawnInLocation( "narratorAtExit", Engine.getPawn( "Narrator" ),
                                                            Engine.getLocation( "Exit Corridor" ) ) );
        ...
        SetSpeakerInitialDialog keysFoundDialog =
            new SetSpeakerInitialDialog( "keysFoundDialog", Engine.getSpeaker( "Narrator" ),
                                         Engine.getDialog( "Narrator Exit Open" ) );
        ...
        InitiateDialog exitFoundDialog = new InitiateDialog( "exitFoundDialog", Engine.getDialog( "Narrator Exit" ) );

        // We need to make sure this dialog only runs once.
        ComponentHasStatus exitDialogPlayed =
            new ComponentHasStatus( "exitDialogPlayed", Engine.getComponent( "Narrator Exit" ), "has run", true );
        ...
        LocationHasExitDirection exitPathAdded =
            new LocationHasExitDirection( "exitPathAdded", Engine.getLocation( "Exit Corridor" ),
                                          Engine.getDirection( "South" ) );
