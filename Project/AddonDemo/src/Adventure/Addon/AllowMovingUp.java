    ...
    extends BaseEvent
{
    ...
    public void processEvent( Operation operation )
    {
        if ( operation != null )
        {
            ...
            {
                if ( !Engine.currentPlayer().hasStatus( "flying" ) )
                {
                    IO.clearBuffer();
                    Engine.currentPlayer().setLocation( Engine.currentPlayer().getPreviousLocation(), false );
                    IO.addLine( "Try as you might, you can't get up there without flying." );
                    Engine.currentPlayer().getLocation().describe();
                }
            }
        }
    }
